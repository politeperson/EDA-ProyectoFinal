...
public:
	...
	unordered_map<char, pair<pair<int, int>, shared_ptr<SUFFIX_NODE>>> getAllTransitions();
...
private:
	...
	unordered_map<char, pair<pair<int, int>, shared_ptr<SUFFIX_NODE>>> trans;
...
unordered_map<char, pair<pair<int, int>, shared_ptr<SUFFIX_NODE>>> SUFFIX_NODE::getAllTransitions()
{
	return trans;
}
...
{
public:
	SUFFIX_TREE(unordered_map<char, int> any_alphabet);
	...
	void getResults(shared_ptr<SUFFIX_NODE>& r, int k, int p, unordered_map<string, int>& results, vector<pair<pair<int, int>, string>>& delimiters);
...
SUFFIX_TREE::SUFFIX_TREE(unordered_map<char, int> any_alphabet)
{
...
void SUFFIX_TREE::update(shared_ptr<SUFFIX_NODE>& s, int& k, int i, string& T) 
{
	...
	while (!testPair.first) 
	{
		...
		testPair.second->setTransition(T[i], i, INT_MAX, nullptr);
		// f'(oldr) = r
...
{
	if (r == nullptr)// si es hoja es un puntero nulo
	{
	...
	while (!dfs.empty())
	{
		...
		for (auto& it_trans : top->getAllTransitions()) {
			if (it_trans.second.second == nullptr)// si es hoja, es un puntero nulo
